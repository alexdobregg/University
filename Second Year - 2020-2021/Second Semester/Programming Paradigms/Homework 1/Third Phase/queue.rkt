...
(define (enqueue x q)
  (struct-copy queue q [right (cons x (queue-right q))] [size-r (+ 1 (queue-size-r q))]))
...
(define (dequeue q)
  (if (null? (queue-left q))
      (struct-copy queue q [left (cdr (reverse (queue-right q)))] [right '()] [size-l (- (queue-size-r q) 1)] [size-r 0])
      (struct-copy queue q [left (cdr (queue-left q))] [size-l (- (queue-size-l q) 1)])))
