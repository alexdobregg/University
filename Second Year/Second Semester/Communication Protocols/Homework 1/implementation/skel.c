...
int read_rtable(struct route_table_entry ***rtable, char *nameRT) {
    char line[100];
	FILE *rt = fopen(nameRT, "rt");
	// Considering both of te rtable.txt files and their number of lines,
	// the memory allocated for the route table is N, which is defined as
	// 64266.
	(*rtable) = (struct route_table_entry **) malloc(N * sizeof(struct route_table_entry *));
	for (int i = 0; i < N; i++) {
		(*rtable)[i] = (struct route_table_entry *) malloc (sizeof(struct route_table_entry));
	}
	int i;
	// Line contains every line from the file.
	for (i = 0; fgets(line, sizeof(line), rt); i++) {
	
		char prefix_str[50], next_hop_str[50], mask_str[50], interface_str[10];
		// We take the prefix, next_hop, mask and interface as strings from the file.
		sscanf(line, "%s %s %s %s", prefix_str, next_hop_str, mask_str, interface_str);
		// Converting Internet host address from numbers-and-dots notation as a string
		// using inet_addr.
		(*rtable)[i]->prefix = inet_addr(prefix_str);
		(*rtable)[i]->next_hop = inet_addr(next_hop_str);
		(*rtable)[i]->mask = inet_addr(mask_str);
		// Convert the interface using atoi from string to int.
...
// free_rtable and free_atable are 2 functions that free the memory allocated
// for route table and arp_table.
void free_rtable(struct route_table_entry **rtable) {
	for (int i = 0; i < N; i++) {
		free(rtable[i]);
	}
	free(rtable);
...
void free_atable(struct arp_table *arp_table) {

	for (int i = 0; i < arp_table->size; i++) {
		free(arp_table->arp_entries[i]);
	}
	free(arp_table->arp_entries);
	free(arp_table);
}

// A function used by quick sort to sort the route table.
int cmp_quick(const void *rt1, const void *rt2) {
	const struct route_table_entry **rt11 = (const struct route_table_entry **) rt1;
	const struct route_table_entry **rt22 = (const struct route_table_entry **) rt2;
	return ((*rt11)->prefix - (*rt22)->prefix);
}

// A function to get the best route for a package.
...
// A function that searches in the arp table for the arp entry.
struct arp_table_entry *get_arp_entry(struct arp_table_entry **arp_entries, int arpL, uint32_t ip) {
    
    for(int i = 0; i < arpL; i++){
